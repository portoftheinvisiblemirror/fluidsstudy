...
void filledmidpointcircle(const double y0, const double z0, const double dy, const double dz, const int ny, const int nz, const double radius, std::vector<std::vector<int>>& circle, const double R, int X)
{
	...
	fillin(circle, floor((z0 + R - radius) / dz), floor((z0 + R + radius) / dz), X, ycenter);
	...
	for (int i = ycenter + 1; i <= yhigh; ++i) //bound this to make sure yhigh is not greater than ny-1?
	{
		...
		//fill in
		fillin(circle, floor((z0 + R - halfz) / dz), floor((z0 + R + halfz) / dz), X, i);
	}
...
std::vector<std::vector<int>> filledmidpointspherex(const double x0, const double y0, const double z0, const double dx, const double dy, const double dz, const int nx, const int ny, const int nz, const double radius, const double R)
{
	...
	filledmidpointcircle(y0, z0, dy, dz, ny, nz, radius, sphere, R, xcenter);
	...
	for (int i = xcenter + 1; i <= xhigh; ++i)
	{
		...
		//fill in
		filledmidpointcircle(y0, z0, dy, dz, ny, nz, halfr, sphere, R,i);
	}
...
void emptiedmidpointcircle(const double y0, const double z0, const double dy, const double dz, const int ny, const int nz, const double radius, const double radlow, std::vector<std::vector<int>>& circle, const double R, int X)
{
	...
	emptiedinside(circle, floor((z0 + R - radius) / dz), floor((z0 + R + radius) / dz), floor((z0 + R - sqrt(radlow * radlow - maxoffset*maxoffset))/dz), floor((z0 + R + sqrt(radlow * radlow - maxoffset*maxoffset))/dz), X, ycenter);
	...
	for (int i = ycenter + 1; i <= yhigh; ++i) //bound this to make sure yhigh is not greater than ny-1?
	{
		...
		//fill in
		emptiedinside(circle, floor((z0 + R - halfz) / dz), floor((z0 + R + halfz) / dz), floor((z0 + R - halflow) / dz), floor((z0 + R + halflow) / dz), X, i);
	}
	for (int i = ycenter - 1; i >= ylow; --i)
	{
		//determine half length
...
std::vector<std::vector<int>> emptiedmidpointspherex(const double x0, const double y0, const double z0, const double dx, const double dy, const double dz, const int nx, const int ny, const int nz, const double radius, const double R)
{
	...
	emptiedmidpointcircle(y0, z0, dy, dz, ny, nz, radius, sqrt(radius * radius -maxoffset*maxoffset), sphere, R,xcenter);
	...
	for (int i = xcenter + 1; i <= xhigh; ++i)
	{
		...
		//fill in
		emptiedmidpointcircle(y0, z0, dy, dz, ny, nz, halfr, halflow, sphere, R,i);
	}
